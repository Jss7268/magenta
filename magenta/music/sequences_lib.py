...
def sequence_to_pianoroll(
    ...
    for note in sorted(sequence.notes, key=lambda n: n.start_time):
        ...
        if (not note.is_drum
                and instrument_family_mappings.midi_instrument_to_family[
                    note.program].value >= timbre_num_classes
                or instrument_family is not None
                and instrument_family_mappings.midi_instrument_to_family[
                    note.program].value != instrument_family):
            # Only add notes for the specified instrument family.
            continue
        elif note.is_drum:
