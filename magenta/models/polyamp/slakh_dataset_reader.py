...
def provide_batch(examples,
    ...
    spec_dataset = reduced_dataset.map(functools.partial(
        timbre_dataset_util.include_spectrogram, hparams=hparams
    ))
    ...
    dataset = model_input.padded_batch(
        ...
        padded_shapes=(
            timbre_dataset_util.FeatureTensors(
                spec=TensorShape([None, 229, 1]),
                note_croppings=TensorShape([None, 3]),
            ),
            timbre_dataset_util.LabelTensors(
        ...
        padding_values=(
            ...
            timbre_dataset_util.LabelTensors(instrument_families=0)),
        drop_remainder=True)
